...
### orientation=:edge — flat-top hexagons (edge up), dense axis is x
...
### orientation=:vertex — pointy-top hexagons (vertex up), dense axis is y
function hex_geometry_vertex(nx_pixels, ny_pixels, gridsize)

    ny_hex = gridsize
    nx_hex = Int(floor(ny_hex / sqrt(3)))
...
function compute_hex_values_vertex(image, gridsize)

    ny, nx = size(image)

    xmin, ymin, sx, sy, nx_hex, ny_hex =
        hex_geometry_vertex(nx, ny, gridsize)
    ...
    for row in 1:ny
        for col in 1:nx
            ...
            d1 = 3 * (ix - ix1)^2 + (iy - iy1)^2
            d2 = 3 * (ix - ix2 - 0.5)^2 + (iy - iy2 - 0.5)^2
...
function hex_fill_image_vertex(image, gridsize)

    ny, nx = size(image)

    values, nx_hex, ny_hex = compute_hex_values_vertex(image, gridsize)
...
function hexbin(input_file::String; output_suffix="-hexbin", orientation::Symbol)
    ...
    elseif orientation === :vertex
        hex_image = hex_fill_image_vertex(image, gridsize)
    else
        error("orientation must be :edge or :vertex")
    end

    hdus[1] = FITSFiles.HDU(hex_image, header)
    output_file = replace(input_file, ".fits" => "$(output_suffix)-$(orientation).fits")
